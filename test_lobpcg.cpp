...
void test_sparse_diag_A() {
    ...
    // ...
    if(!ok) std::cerr<< "not ok! "<< std::endl;
}
...
void test_a1_9() {
    ...
    int n_max_subspace = 2;//10; // n_eig = min(2*n_want, n_want + 5)
    bool solving_generalized = false;
    int max_iter = 100;
    double tol = 1e-6;
    double shift = 0.0;
    ...
    Eigen::MatrixXd evec(n, n_max_subspace);
    eig.setZero(); evec.setZero();
    int ok = lobpcg_solve(_no_matvec/*a1vec*/, _no_matvec, bvec, eig, evec, n, n_eigenpairs, n_max_subspace, solving_generalized, max_iter, tol, shift, verbose);
